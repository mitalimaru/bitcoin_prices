...
#Forecasting
library(forecast)
ts_data <- ts(BTC_Ecotrix_Data$Price_Bitcoin, start= c(2017,1), frequency = 12)
train_data <- window(ts_data, start = c(2017,1), end = c(2022,6))
test_data <- window(ts_data, start = c(2022,7))

Model1HH <- arima(train_data, order = c(0,1,7))
Model2HH <- arima(train_data, order = c(7,1,0))
forecast1 <- forecast(Model1HH, h = length(test_data))
forecast2 <- forecast(Model2HH, h = length(test_data))
